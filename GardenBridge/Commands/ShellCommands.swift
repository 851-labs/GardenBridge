...
actor ShellCommands: CommandExecutor {
  ...
  func execute(command: String, params: [String: AnyCodable]) async throws -> AnyCodable? {
    switch command {
    case "shell.execute":
      return try await self.executeShell(params: params)
    case "shell.which":
      return try await self.whichCommand(params: params)
    default:
  ...
  private func executeShell(params: [String: AnyCodable]) async throws -> AnyCodable {
    ...
    if let cwd = workingDirectory {
      process.currentDirectoryURL = URL(fileURLWithPath: self.expandPath(cwd))
    }

    // Set environment if specified
    if let env = environment {
      process.environment = self.mergedEnvironment(with: env)
    }
  ...
  private func whichCommand(params: [String: AnyCodable]) async throws -> AnyCodable {
    ...
    let process = Process()
    process.executableURL = self.whichURL
