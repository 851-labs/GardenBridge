...
actor FileSystemCommands: CommandExecutor {
    ...
    func execute(command: String, params: [String: AnyCodable]) async throws -> AnyCodable? {
        switch command {
        case "file.read":
            return try await readFile(params: params)
        case "file.write":
            return try await writeFile(params: params)
        case "file.list":
            return try await listDirectory(params: params)
        case "file.exists":
            return try await fileExists(params: params)
        case "file.delete":
            return try await deleteFile(params: params)
        case "file.info":
            return try await fileInfo(params: params)
        default:
    ...
    private func writeFile(params: [String: AnyCodable]) async throws -> AnyCodable {
        ...
        if params["binary"]?.boolValue == true {
            ...
            if append && fileManager.fileExists(atPath: expandedPath) {
