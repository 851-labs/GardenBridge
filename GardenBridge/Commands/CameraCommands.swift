...
actor CameraCommands: CommandExecutor {
  ...
  func execute(command: String, params: [String: AnyCodable]) async throws -> AnyCodable? {
    switch command {
    case "camera.snap":
      return try await self.capturePhoto(params: params)
    case "camera.list":
      return try await self.listCameras()
    default:
  ...
  private func capturePhoto(params: [String: AnyCodable]) async throws -> AnyCodable {
    ...
    let photo = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<
      AVCapturePhoto,
      Error,
    >) in
      ...
      Task {
        await self.storeActiveDelegate(delegate)
      }
...
private class PhotoCaptureDelegate: NSObject, AVCapturePhotoCaptureDelegate, @unchecked Sendable {
  ...
  func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
    if let error {
      ...
      self.continuation?.resume(returning: photo)
    }
