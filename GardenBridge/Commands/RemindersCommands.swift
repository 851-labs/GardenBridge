...
actor RemindersCommands: CommandExecutor {
    ...
    func execute(command: String, params: [String: AnyCodable]) async throws -> AnyCodable? {
        switch command {
        case "reminders.list":
            return try await self.listReminders(params: params)
        case "reminders.create":
            return try await self.createReminder(params: params)
        case "reminders.complete":
            return try await self.completeReminder(params: params)
        case "reminders.delete":
            return try await self.deleteReminder(params: params)
        case "reminders.getLists":
            return try await self.getReminderLists()
        default:
    ...
    private func listReminders(params: [String: AnyCodable]) async throws -> AnyCodable {
        ...
        let calendars: [EKCalendar]? = if let listName {
            self.eventStore.calendars(for: .reminder).filter { $0.title == listName }
        } else {
            self.eventStore.calendars(for: .reminder)
        }

        let predicate: NSPredicate = if includeCompleted {
            self.eventStore.predicateForReminders(in: calendars)
        } else {
            self.eventStore.predicateForIncompleteReminders(withDueDateStarting: nil, ending: nil, calendars: calendars)
        }
    ...
    private func getReminderLists() async throws -> AnyCodable {
        ...
        let listDicts = calendars.map { calendar -> [String: Any] in
            [
                ...
                "isDefault": calendar == self.eventStore.defaultCalendarForNewReminders(),
