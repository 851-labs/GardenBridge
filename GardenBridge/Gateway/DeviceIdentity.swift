...
actor DeviceIdentity {
    ...
    init() {
        Task {
            await self.loadOrCreateIdentity()
        }
    }
    ...
    func getDeviceId() async -> String {
        ...
        await self.loadOrCreateIdentity()
        return self.deviceId ?? UUID().uuidString
    }
    ...
    private func loadOrCreateIdentity() async {
        ...
        if let existingKey = loadKeyFromFile() {
            ...
            self.deviceId = self.generateDeviceId(from: existingKey.publicKey)
            return
        }
        ...
        self.deviceId = self.generateDeviceId(from: newKey.publicKey)

        self.saveKeyToFile(newKey)
    }
    ...
    private func saveKeyToFile(_ key: P256.Signing.PrivateKey) {
        ...
        do {
            ...
            let directory = self.keyFileURL.deletingLastPathComponent()
            try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)

            // Write key data with restricted permissions
            try keyData.write(to: self.keyFileURL, options: [.atomic, .completeFileProtection])
