---
name: release
description: Release GardenBridge macOS app. Bumps version, updates changelog, creates git tag, monitors CI, and verifies release assets. Use when releasing a new version.
---

# GardenBridge Release Process

Follow these steps to ship a new macOS release for GardenBridge.
...
```bash
git add GardenBridge.xcodeproj/project.pbxproj
...
1. Download the DMG and compute SHA256:
   ...
   shasum -a 256 /tmp/GardenBridge.dmg
   ```

2. Update `851-labs/homebrew-tap` repository:
   - Edit `Casks/gardenbridge.rb`
   - Set `version "X.Y.Z"`
...
4. Validate the install:
   ```bash
   brew update
   brew upgrade --cask 851-labs/tap/gardenbridge
   ```
...
```
https://github.com/851-labs/GardenBridge/releases/latest/download/appcast.xml
```
